...
public class OrderServiceImpl implements OrderService {
    ...
    public OrderSubmitVO submit(OrdersSubmitDTO orderSubmitDTO) {
        ...
        for (ShoppingCart cart : shoppingCartList) {
            ...
            orderDetailList.add(orderDetail); // 订单明细添加到订单明细列表
        }
    ...
    public void userCancelById(Long id) throws Exception {
        ...
        // 检查订单的状态 1待付款 2待接单 3已接单 4派送中 5已完成 6已取消
    ...
    public void rejection(OrdersRejectionDTO ordersRejectionDTO) {
        ...
        if (payStatus == Orders.PAID) {
            //用户已支付，需要退款
            String refund = String.valueOf(weChatPayUtil.refund(
                    ordersDB.getNumber(),
                    ordersDB.getNumber(),
                    new BigDecimal(0.01),
                    new BigDecimal(0.01)));
    ...
    public void reminder(Long id) {
        ...
        Map map = new HashMap<>();
        map.put("type",2);   // 1 表示来电提醒   2 表示客户催单
        map.put("orderId",id);
        map.put("content","订单号：" + ordersDB.getNumber());

        // 通过webSocketServer发送消息到所有客户端
