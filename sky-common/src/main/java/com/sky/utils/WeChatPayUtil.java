...
public class WeChatPayUtil {
     ...
     * 假支付接口：不会真正调用微信
     */
    public JSONObject pay(String orderNo, BigDecimal amount, String description, String openId) {

        JSONObject result = new JSONObject();

        // —— 你业务判断需要的字段 ——
        result.put("code", "SUCCESS");  // 只要不是 ORDERPAID 就不会进入异常
        result.put("message", "模拟支付下单成功");
        ...
        result.put("package", "prepay_id=" + UUID.randomUUID().toString().replace("-", ""));
        result.put("signType", "MD5");
        result.put("paySign", "MOCK_SIGN_" + UUID.randomUUID().toString().replace("-", ""));
